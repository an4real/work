...
def clean_names_gemini(df, output_file="cleaned_names.csv"):
    ...
    requests_per_minute = 2  #Ограничение по кличеству запросов. Для gemini 2.0 можно 500, для gemini 1.5 лучше 200
    ...
    for i in range(0, len(df), batch_size):
        ...
        #Промт. Советую обязательно давать модели примеры 
        prompt = f"""
         Очисти поле 'Name' от всего лишнего, оставив только ФИО на русском. Убери лишние слова, кроме не ФИО.
        ...
        Пример: 'ОМВД России по Вязниковскому району участок. Для получения информации обратитесь к начальнику пункта полиции ОУУП майору  Куприну Дмитрию Вячеславовичу тел'. Результат: Куприн Дмитрий Вячеславович.
...
#что не обработались, прогоняем через этот конвеер, заменяем их в исходном датафрейме. 
#Модель часто пропускает строки, поэтому советую делать батчи как можно меньше (в идеале - 200 строк)
